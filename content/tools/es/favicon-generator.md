...
Es común preguntarse: *¿Por qué seguimos usando `favicon.ico` si hoy tenemos [PNG](/es/herramientas/convert-to-png) y SVG?*
...
A diferencia de un [PNG](/es/herramientas/convert-to-png) o [JPEG](/es/herramientas/convert-to-jpg) estándar que guarda una sola imagen, el formato ICO fue diseñado como un "directorio" de recursos binarios. Puede albergar múltiples imágenes con diferentes tamaños y profundidades de color dentro de un mismo archivo físico.
...
Los archivos `apple-touch-icon.png` **nunca deben contener transparencia**. Si usted proporciona a iOS un ícono [PNG](/es/herramientas/convert-to-png) con fondo transparente (Canal Alfa), Apple llenará todo ese vacío con un color negro puro y sólido. Esto arruinará la apariencia de su logotipo web. Por ello, nuestra herramienta le permite seleccionar un "Fondo Sólido" (Solid Background) rellenando la capa inferior con blanco o un color Hexagonal corporativo, preservando la estética.
