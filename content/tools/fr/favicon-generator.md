...
On entend souvent : *"Pourquoi m'embêter avec un fichier .ico obsolète si on a inventé le format [PNG](/fr/outils/convert-to-png) ?"*
...
Notre générateur compile ce conteneur en associant vos différents calques [PNG](/fr/outils/convert-to-png) et en encodant les adresses binaires (Byte Offsets), créant un `.ico` totalement valide via une compilation `Client-Side`.
...
Le fichier `apple-touch-icon.png` **NE DOIT PAS** comporter de canal alpha (transparence). Si vous donnez un [PNG](/fr/outils/convert-to-png) transparent à l'iPhone, iOS va saturer le vide avec une immonde couleur noire solide.
