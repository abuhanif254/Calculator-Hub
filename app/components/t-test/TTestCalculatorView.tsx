...
import { TTestBasicMode } from "./TTestBasicMode";
import { TTestDatasetAnalyzer } from "./TTestDatasetAnalyzer";
import { TTestExplorerMode } from "./TTestExplorerMode";
import { TTestLearningSuite } from "./TTestLearningSuite";

interface TTestCalculatorViewProps {
  calcDef: CalculatorDef;
}

type TabMode = "calculator" | "dataset" | "explorer" | "learning";

export function TTestCalculatorView({ calcDef }: TTestCalculatorViewProps) {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-200 overflow-hidden">
      ...
      <div className="p-6 md:p-8">
        {activeTab === "calculator" && <TTestBasicMode />}
        {activeTab === "dataset" && <TTestDatasetAnalyzer />}
        {activeTab === "explorer" && <TTestExplorerMode />}
        {activeTab === "learning" && <TTestLearningSuite />}
      </div>
    </div>
  );
}
