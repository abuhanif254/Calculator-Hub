...
export function HendersonHasselbalchCalculatorView() {
  const [isClient, setIsClient] = useState(false);
  const [calcMode, setCalcMode] = useState<CalculationMode>("calculate_ph");
  ...
  const [inputTargetPH, setInputTargetPH] = useState<number>(4.76);
  ...
  const results = useMemo(() => {
    ...
    if (calcMode === "calculate_ph") {
      ...
      methodNote = "pH = pKa + log10([A-]/[HA])";
    } else if (calcMode === "calculate_pka") {
      calculatedPH = inputTargetPH;
      calculatedBase = Math.max(1e-6, inputConjugateBaseConc);
      calculatedAcid = Math.max(1e-6, inputWeakAcidConc);
      calculatedRatio = calculatedBase / calculatedAcid;
      calculatedPKa = calculatedPH - Math.log10(calculatedRatio);
      methodNote = "pKa = pH - log10([A-]/[HA])";
    } else if (calcMode === "calculate_ratio") {
      calculatedPH = inputTargetPH;
      calculatedPKa = pKaVal;
      calculatedRatio = Math.pow(10, calculatedPH - calculatedPKa);
      calculatedAcid = 0.10;
      calculatedBase = 0.10 * calculatedRatio;
      methodNote = "[A-]/[HA] = 10^(pH - pKa)";
    } else if (calcMode === "calculate_conjugate_base") {
      calculatedPH = inputTargetPH;
      calculatedPKa = pKaVal;
      calculatedAcid = Math.max(1e-6, inputWeakAcidConc);
      calculatedRatio = Math.pow(10, calculatedPH - calculatedPKa);
      calculatedBase = calculatedAcid * calculatedRatio;
      methodNote = "[A-] = [HA] * 10^(pH - pKa)";
    } else if (calcMode === "calculate_weak_acid") {
      calculatedPH = inputTargetPH;
      calculatedPKa = pKaVal;
      calculatedBase = Math.max(1e-6, inputConjugateBaseConc);
      calculatedRatio = Math.pow(10, calculatedPH - calculatedPKa);
      calculatedAcid = calculatedBase / calculatedRatio;
      methodNote = "[HA] = [A-] / 10^(pH - pKa)";
    } else {
      calculatedBase = Math.max(1e-6, inputConjugateBaseConc);
      calculatedAcid = Math.max(1e-6, inputWeakAcidConc);
      calculatedRatio = calculatedBase / calculatedAcid;
      calculatedPKa = pKaVal;
      calculatedPH = calculatedPKa + Math.log10(calculatedRatio);
      methodNote = "Standard Henderson-Hasselbalch Equation";
    }
    ...
    if (delta <= 1.0) bufferStatus = "Optimal Buffer (within pKa ± 1.0)";
  ...
  }, [calcMode, inputPKa, inputConjugateBaseConc, inputWeakAcidConc, inputTargetPH, kwInfo]);
  ...
  // Flashcards Data
  const flashcards = [
    { title: "Henderson-Hasselbalch Equation", desc: "pH = pKa + log10([A-]/[HA]). Relates buffer solution pH to weak acid pKa and conjugate pair concentrations." },
    { title: "Optimal Buffer Condition", desc: "When [A-] = [HA], log10(1) = 0, so pH = pKa. This yields maximum buffer capacity beta." },
    { title: "Effective Buffer Range", desc: "The useful buffer range is typically pKa ± 1.0 pH unit, corresponding to ratios between 0.1 and 10." },
    { title: "Buffer Capacity (beta)", desc: "beta = 2.303 * Ctotal * (Ka*[H+]) / (Ka+[H+])^2. Measures resistance to pH changes when acid or base is added." }
  ];

  // Handlers
  const handleCopySummary = () => {
    const text = `Henderson-Hasselbalch & Buffer Analysis Summary
------------------------------------------------
Selected Mode: ${calcMode}
Temperature: ${temperatureC}°C (pKw = ${results.pkw.toFixed(2)})
Calculated pH: ${results.calculatedPH.toFixed(2)}
Acid pKa: ${results.calculatedPKa.toFixed(2)}
Conjugate Base Concentration [A-]: ${results.calculatedBase.toFixed(4)} M
Weak Acid Concentration [HA]: ${results.calculatedAcid.toFixed(4)} M
...
Calculation Method: ${results.methodNote}`;
    ...
    navigator.clipboard.writeText(text).then(() => {
      triggerNotification("success", "Buffer analysis summary copied to clipboard!");
    });
  };
  ...
  const generateQuiz = () => {
    const questions = [
      { q: "What is the pH of a buffer with pKa = 4.76 when [A-] = 0.10 M and [HA] = 0.10 M?", correctAnswer: "4.76" },
      { q: "True or False: Maximum buffer capacity occurs when pH = pKa.", correctAnswer: "TRUE" },
      { q: "What is the [A-]/[HA] ratio when pH = pKa + 1.00?", correctAnswer: "10" }
    ];
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200 dark:border-slate-800">
            ...
            <div className="space-y-2 mb-6">
              ...
              >
                <option value="calculate_ph">1. Calculate Buffer pH from pKa, [A-], and [HA]</option>
                <option value="calculate_pka">2. Calculate pKa from Target pH and Buffer Concentrations</option>
                <option value="calculate_ratio">3. Calculate [A-]/[HA] Ratio from Target pH & pKa</option>
                <option value="calculate_conjugate_base">4. Calculate Conjugate Base [A-] from pH, pKa, & [HA]</option>
                <option value="calculate_weak_acid">5. Calculate Weak Acid [HA] from pH, pKa, & [A-]</option>
                <option value="buffer_capacity">6. Buffer Capacity Analysis (beta)</option>
                <option value="species_distribution">7. Species Distribution Analysis</option>
              </select>
            </div>
            ...
            <div className="space-y-5">
              ...
              {calcMode !== "calculate_ph" && (
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-sm font-semibold text-slate-700 dark:text-slate-300">
                    <span>Target Buffer pH</span>
                    <input
                      type="number"
                      step="0.01"
                      value={inputTargetPH}
                      onChange={(e) => setInputTargetPH(Number(e.target.value))}
                      className="w-24 bg-slate-50 dark:bg-slate-950 text-right p-1.5 rounded-xl border border-slate-200 dark:border-slate-800 font-black text-xs"
                    />
                  </div>
                </div>
              ...
              {(calcMode === "calculate_ph" || calcMode === "calculate_pka" || calcMode === "calculate_weak_acid" || calcMode === "buffer_capacity" || calcMode === "species_distribution") && (
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-sm font-semibold text-slate-700 dark:text-slate-300">
              ...
              {(calcMode === "calculate_ph" || calcMode === "calculate_pka" || calcMode === "calculate_conjugate_base" || calcMode === "buffer_capacity" || calcMode === "species_distribution") && (
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-sm font-semibold text-slate-700 dark:text-slate-300">
              ...
              />
              <ReadOnlyField 
                label="[A-]/[HA] Conjugate Ratio" 
                val={results.calculatedRatio.toFixed(3)} 
                icon={Zap} 
              />
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 shadow-sm border border-slate-200 dark:border-slate-800 flex items-center justify-between">
            <div>
              <span className="text-xs font-bold text-slate-900 dark:text-white block">Looking for acid dissociation constants?</span>
              <span className="text-[11px] text-slate-500">Try our companion pKa Calculator</span>
            </div>
            <Link
              href={resolveIntlHref("/calculators/pka-calculator")}
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden animate-fade-in">
              ...
              <div className="flex border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-1 flex-wrap">
                {(["buffer_range", "capacity_chart", "species_chart", "flashcards", "quiz"] as const).map(tab => (
                  ...
                  >
                    {tab === "buffer_range" ? "🛡️ Range Model" : tab === "capacity_chart" ? "📈 Buffer Capacity" : tab === "species_chart" ? "📊 Species Distribution" : tab === "flashcards" ? "🎴 Flashcards" : "Quiz"}
                  </button>
                ))}
              </div>
              ...
              <div className="p-6">
                ...
                {visualTab === "buffer_range" && (
                  <div className="space-y-3 text-xs text-slate-600 dark:text-slate-400">
                    <h4 className="text-sm font-bold text-slate-900 dark:text-white">Henderson-Hasselbalch Mathematical Model</h4>
                ...
                {visualTab === "flashcards" && (
                  <div className="space-y-4">
                    <h4 className="text-sm font-bold text-slate-900 dark:text-white">Henderson-Hasselbalch Study Flashcards</h4>
                    <div 
                ...
                {visualTab === "quiz" && (
                  <div className="space-y-4">
                    <h4 className="text-sm font-bold text-slate-900 dark:text-white">Henderson-Hasselbalch Quiz</h4>
                    {quizQuestion && (
