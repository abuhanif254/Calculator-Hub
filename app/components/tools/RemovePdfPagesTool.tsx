...
interface UploadedPdf {
  ...
  errorMessage?: string;
  pdfjsDoc: any; // PDFJS Document object for thumbnails
  selectedPages: boolean[]; // true = selected by user (can mean keeping or deleting depending on mode)
  rangeString: string;
  metadata: PdfMetadata | null;
}

interface RemovePagesHistoryItem {
  id: string;
  timestamp: number;
  inputFiles: { name: string; size: number; pageCount: number; pagesRemoved: number }[];
  totalRemoved: number;
  deleteMode: string;
  totalSize: number;
}
...
function Thumbnail({ pageNumber, pdfDoc, zoom, isSelected, onToggleSelect }: ThumbnailProps) {
  ...
  return (
    ...
    >
      {/* Visual Checkbox Indicator (Red Trash if selected for removal, Green if kept) */}
      <button
        type="button"
        onClick={(e) => {
          e.stopPropagation();
      ...
      <div className="w-32 h-44 flex items-center justify-center bg-white dark:bg-slate-900 rounded-lg overflow-hidden border border-slate-200/50 dark:border-slate-800/50 relative shadow-3xs">
        ...
        {renderStatus === "rendering" && (
          <div className="flex flex-col items-center gap-1">
            ...
            <span className="text-[8px] text-slate-400 dark:text-slate-500 font-semibold uppercase">Drawing</span>
          </div>
        )}
        {renderStatus === "error" && (
        ...
        {renderStatus === "done" && (
          <div className="absolute inset-0 bg-slate-950/5 dark:bg-slate-950/20 opacity-0 group-hover/thumb:opacity-100 transition-opacity flex items-center justify-center pointer-events-none">
            <div className="bg-[#518231] text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full shadow-md">
              Page {pageNumber}
            </div>
          </div>
        )}
...
export function RemovePdfPagesTool() {
  ...
  // Settings
  const [deleteMode, setDeleteMode] = useState<"remove_selected" | "keep_selected" | "odd" | "even" | "blank">("remove_selected");
  const [outputNamePattern, setOutputNamePattern] = useState<string>("{filename}_edited");
  ...
  const [historyList, setHistoryList] = useState<RemovePagesHistoryItem[]>([]);
  ...
  useEffect(() => {
    const saved = localStorage.getItem("remove_pdf_pages_history");
    if (saved) {
      try {
        setHistoryList(JSON.parse(saved));
      } catch (e) {
        console.error("Failed to parse local history for remove PDF pages", e);
      }
    }
  }, []);
  ...
  const loadPdfJs = useCallback((): Promise<any> => {
    return new Promise((resolve, reject) => {
      ...
      script.src = `https://cdnjs.cloudflare.com/ajax/libs/pdf.js/${PDFJSJS_VERSION()}/pdf.min.js`;
      function PDFJSJS_VERSION() { return PDFJS_VERSION; }
      script.onload = () => {
  ...
  const processFiles = async (uploadedFiles: FileList) => {
    ...
    // Set active file to the first uploaded one if none active
    if (!activeFileId) {
      setActiveFileId(list[0].id);
    }

    // Lazy load the PDFJS library
    let pdfjs: any = null;
    try {
    ...
    // Parse files one by one in the background
  ...
  const togglePageSelection = (fileId: string, index: number) => {
    setFiles(prev => prev.map(f => {
      if (f.id === fileId) {
  ...
  // Apply visual preset selection based on filters
  const applyPresetFilter = (fileId: string, mode: "odd" | "even") => {
    setFiles(prev => prev.map(f => {
      if (f.id === fileId && f.pageCount) {
  ...
  // Handle textual range input edits
  const handleRangeTextChange = (fileId: string, value: string) => {
    setFiles(prev => prev.map(f => {
      if (f.id === fileId && f.pageCount) {
        // Find indices matching this typed input range string
  ...
  // Calculate statistics: output name, removed pages indices, kept page indices
  const getFileStats = (f: UploadedPdf) => {
    if (!f.pageCount) return { keptCount: 0, removedCount: 0, keptIndices: [] as number[], removedIndices: [] as number[] };

    let keptIndices: number[] = [];
    let removedIndices: number[] = [];

    // Parse depending on delete mode
    if (deleteMode === "remove_selected") {
      f.selectedPages.forEach((selected, idx) => {
        if (selected) removedIndices.push(idx);
        else keptIndices.push(idx);
      });
    } else if (deleteMode === "keep_selected") {
      f.selectedPages.forEach((selected, idx) => {
        if (selected) keptIndices.push(idx);
        else removedIndices.push(idx);
      });
    } else if (deleteMode === "odd") {
      f.selectedPages.forEach((_, idx) => {
        const isOdd = (idx + 1) % 2 !== 0;
        if (isOdd) removedIndices.push(idx);
        else keptIndices.push(idx);
      });
    } else if (deleteMode === "even") {
      f.selectedPages.forEach((_, idx) => {
        const isEven = (idx + 1) % 2 === 0;
        if (isEven) removedIndices.push(idx);
        else keptIndices.push(idx);
      });
    } else if (deleteMode === "blank") {
      // Placeholder: blank pages removal behaves like "remove selected" for now,
      // simulating blank pages marked by the user.
      f.selectedPages.forEach((selected, idx) => {
        if (selected) removedIndices.push(idx);
        else keptIndices.push(idx);
      });
  ...
  // Compiling Action - Local PDF generation
  const handleRemovePages = async () => {
    const readyFiles = files.filter(f => f.status === "ready");
    if (readyFiles.length === 0) return;

    // Validate that we aren't deleting ALL pages of a document
    for (const f of readyFiles) {
      const { keptCount } = getFileStats(f);
      if (keptCount === 0) {
        setErrorMessage(`Cannot remove all pages from "${f.name}". A PDF must retain at least one page.`);
        return;
      }
    }
    ...
    setProcessingStep("Reading files and parsing catalog references...");
    setErrorMessage(null);

    try {
      const processedFiles: { name: string; bytes: Uint8Array; originalFile: UploadedPdf; removedPages: number }[] = [];
      ...
      for (let i = 0; i < readyFiles.length; i++) {
        ...
        // Retrieve indices to keep
        const { keptIndices, removedCount } = getFileStats(f);

        // Compile output PDF using pdf-lib page copy mechanism
        const outDoc = await PDFDocument.create();
        const copiedPages = await outDoc.copyPages(srcDoc, keptIndices);
        copiedPages.forEach(p => outDoc.addPage(p));

        // Preserve metadata if enabled
        if (preserveMetadata && f.metadata) {
          if (f.metadata.title) outDoc.setTitle(f.metadata.title);
          if (f.metadata.author) outDoc.setAuthor(f.metadata.author);
          if (f.metadata.creator) outDoc.setCreator(f.metadata.creator);
          if (f.metadata.subject) outDoc.setSubject(f.metadata.subject);
          if (f.metadata.producer) outDoc.setProducer(f.metadata.producer);
        }

        // Apply visual compression parameters
        const pdfBytes = await outDoc.save({
          useObjectStreams: true,
          addDefaultPage: false
        });

        // Resolve custom file name pattern
      ...
      if (!isMultiple) {
        ...
        finalName = "cleaned_documents.zip";
      ...
      // Write to history
      const historyItem: RemovePagesHistoryItem = {
        id: `remove-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
        timestamp: Date.now(),
        inputFiles: processedFiles.map(pf => ({
          name: pf.originalFile.name,
          size: pf.originalFile.size,
          pageCount: pf.originalFile.pageCount || 0,
          pagesRemoved: pf.removedPages
        })),
        totalRemoved: processedFiles.reduce((acc, pf) => acc + pf.removedPages, 0),
        deleteMode: deleteMode === "remove_selected" ? "Remove Selected" :
                    deleteMode === "keep_selected" ? "Keep Selected" :
                    deleteMode === "odd" ? "Remove Odd Pages" :
                    deleteMode === "even" ? "Remove Even Pages" : "Remove Blank Pages",
        totalSize: totalOutputSize
      };

      setHistoryList(prev => {
        const updated = [historyItem, ...prev].slice(0, 50); // limit to 50
        localStorage.setItem("remove_pdf_pages_history", JSON.stringify(updated));
        return updated;
      });

    } catch (e: any) {
      console.error(e);
      setErrorMessage("An error occurred during page removal: " + (e.message || "Unknown error"));
      setIsProcessing(false);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div className="space-y-1">
          <span className="text-[10px] font-extrabold text-[#518231] uppercase tracking-widest flex items-center gap-1.5">
            <ShieldCheck size={12} />
            SECURE CLIENT-SIDE DOCUMENT CLEANER
          </span>
          <h1 className="text-xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
            <FileMinus size={22} className="text-[#518231]" />
            Remove PDF Pages
          </h1>
      ...
      {activeTab === "history" ? (
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xs space-y-6">
          <div className="flex items-center justify-between">
            ...
            {historyList.length > 0 && (
              <button
                onClick={() => {
                  localStorage.removeItem("remove_pdf_pages_history");
                  setHistoryList([]);
                }}
          ...
          {historyList.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-slate-400 dark:text-slate-500 gap-3 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-slate-500">Your processed files will be listed here locally.</p>
              </div>
            </div>
          ) : (
                ...
                <div key={item.id} className="py-4 first:pt-0 last:pb-0 flex flex-col md:flex-row justify-between gap-4 text-xs">
                  <div className="space-y-1">
                    ...
                    <div className="text-[10px] text-slate-400 flex flex-wrap gap-x-3 gap-y-1 font-semibold">
                      ...
                      <span>Mode: {item.deleteMode}</span>
                      <span>Removed: {item.totalRemoved} pages</span>
        ...
        <div className="space-y-8">
          ...
          {files.length === 0 ? (
            <div
              ...
              className={`border-3 border-dashed rounded-3xl p-10 flex flex-col items-center justify-center gap-4 text-center cursor-pointer transition-all duration-300 ${
                dragActive
                  ...
                  : "border-slate-300 hover:border-[#518231]/70 dark:border-slate-750 dark:hover:border-[#518231]/70 bg-white hover:bg-slate-50/50 dark:bg-slate-900 dark:hover:bg-slate-900/60"
              }`}
            >
              <input
                ref={fileInputRef}
            ...
            // Uploaded list layout
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              
              {/* Left Column: Upload List & Batch Management */}
              <div className="lg:col-span-4 space-y-6">
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-xs space-y-5">
                  ...
                  <div className="space-y-2 max-h-[250px] overflow-y-auto custom-scrollbar pr-1">
                    {files.map((f) => {
                      ...
                      const { keptCount, removedCount } = getFileStats(f);
                      return (
                        <div
                          key={f.id}
                          onClick={() => {
                              ...
                              <div className="text-[10px] text-slate-400 dark:text-slate-500 flex justify-between font-bold">
                                ...
                                {f.status === "ready" && (
                                  <span className="text-slate-500">
                                    {f.pageCount} pgs ({keptCount} keep / {removedCount} rm)
                                  </span>
                                )}
                              </div>
                            </div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-xs space-y-5">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider block">Delete Mode</label>
                    <div className="space-y-1">
                      {[
                        { id: "remove_selected", label: "Remove Selected Pages", desc: "Deletes checked pages; retains unselected." },
                        { id: "keep_selected", label: "Keep Selected Pages Only", desc: "Retains checked pages; discards others." },
                        { id: "odd", label: "Remove Odd Pages", desc: "Deletes pages 1, 3, 5, 7, etc." },
                        { id: "even", label: "Remove Even Pages", desc: "Deletes pages 2, 4, 6, 8, etc." },
                        { id: "blank", label: "Remove Blank Pages (Future)", desc: "Visual placeholders (coming soon)." }
                      ].map((mode) => (
                        <label
                          key={mode.id}
                          className={`flex items-start gap-2.5 p-2 rounded-xl border text-left cursor-pointer transition-all ${
                            deleteMode === mode.id
                              ? "border-[#518231] bg-green-50/15 dark:bg-[#518231]/5"
                              : "border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-950/20"
                          }`}
                        >
                          <input
                            type="radio"
                            name="deleteMode"
                            checked={deleteMode === mode.id}
                            disabled={mode.id === "blank"} // Disabled for future implementation
                            onChange={() => setDeleteMode(mode.id as any)}
                            className="mt-1 accent-[#518231] shrink-0"
                          />
                          <div className="space-y-0.5">
                            <span className="text-[11px] font-black text-slate-700 dark:text-slate-250 block">
                              {mode.label}
                            </span>
                  ...
                  <div className="flex items-center justify-between p-2 rounded-xl bg-slate-50 dark:bg-slate-950/30 border border-slate-150 dark:border-slate-800">
                    <div className="space-y-0.5">
                      ...
                      <span className="text-[8px] text-slate-400 dark:text-slate-500 block">Copy author, creator, and title details</span>
                    </div>
                    <button
                ...
                {/* Compilation Actions Panel */}
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-xs space-y-4">
                  <h3 className="text-xs font-extrabold text-slate-800 dark:text-slate-200 uppercase tracking-wider border-b border-slate-100 dark:border-slate-800 pb-2">
                    Compile Output
                  </h3>

                  {/* Progress info if compiling */}
                  {isProcessing ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="bg-slate-50 dark:bg-slate-950/40 border border-slate-200/60 dark:border-slate-800/80 rounded-xl p-3 space-y-2">
                        <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest block">Batch Summary</span>
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs font-bold text-slate-700 dark:text-slate-350">
                          ...
                          <div>
                            <span className="text-slate-400 font-semibold block text-[10px]">PAGES FOR REMOVAL</span>
                            <span className="text-red-500 font-black">
                              {files.reduce((acc, f) => acc + getFileStats(f).removedCount, 0)}
                            </span>
                          </div>
                          <div>
                            <span className="text-slate-400 font-semibold block text-[10px]">REMAINING PAGES</span>
                            <span className="text-green-600 dark:text-green-400 font-black">
                              {files.reduce((acc, f) => acc + getFileStats(f).keptCount, 0)}
                            </span>
                          </div>
                          <div>
                            <span className="text-slate-400 font-semibold block text-[10px]">ESTIMATED SIZE</span>
                            <span>
                              {formatBytes(
                                files.reduce((acc, f) => {
                                  const stats = getFileStats(f);
                                  // Simple proportional estimate based on page ratios
                                  const ratio = f.pageCount ? stats.keptCount / f.pageCount : 1.0;
                                  return acc + (f.size * ratio);
                                }, 0)
                              )}
                            </span>
                          </div>
                      ...
                      {/* Download button if compiled ready */}
                      {downloadBlobUrl ? (
                        <div className="space-y-2">
                          <a
                          ...
                          >
                            <Download size={14} />
                            Download {downloadType === "zip" ? "ZIP Archive" : "Clean PDF"}
                          </a>
                          <button
                            onClick={() => {
                            ...
                            className="w-full py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-750 text-slate-600 dark:text-slate-400 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1"
                          >
                ...
                <div className="bg-slate-50 dark:bg-slate-950/20 border border-slate-200/50 dark:border-slate-850 p-4 rounded-2xl flex gap-3 text-left">
                  ...
                  <div className="space-y-0.5">
                    <h4 className="text-[11px] font-bold text-slate-850 dark:text-slate-200 uppercase tracking-wide">Your PDFs remain private</h4>
                    <p className="text-[9px] text-slate-400 dark:text-slate-500 leading-normal font-semibold">
                      Files never leave your browser. All page extraction and compilation operations are executed entirely on your machine.
                    </p>
                  </div>
                </div>
              ...
              {/* Right Column: Visual Page Selection Grid */}
              <div className="lg:col-span-8 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xs min-h-[500px] flex flex-col">
                ...
                {activeFile ? (
                  <div className="flex-1 flex flex-col space-y-6">
                    ...
                    {/* Header: File navigation details */}
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
                      <div className="text-left space-y-0.5">
                        <div className="text-xs text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest">Active File</div>
                        <h2 className="text-sm font-black text-slate-800 dark:text-slate-100 truncate max-w-[300px]">
                          {activeFile.name}
                        </h2>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-wide">
                        ...
                        <button
                          onClick={() => applyPresetFilter(activeFile.id, "odd")}
                        ...
                        <button
                          onClick={() => applyPresetFilter(activeFile.id, "even")}
                          className="px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-850 dark:hover:bg-slate-800 text-[#518231] rounded-lg transition-all"
                        >
                          Evens
                        </button>
                    ...
                    {/* Controls: Zoom, range input */}
                    <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 bg-slate-50 dark:bg-slate-950/20 border border-slate-150 dark:border-slate-850 p-3 rounded-xl">
                      
                      {/* Text Range input field */}
                      <div className="flex-1 flex flex-col md:flex-row items-start md:items-center gap-2 text-left">
                      ...
                      <div className="flex items-center gap-2 self-end md:self-auto shrink-0">
                        <button
                          ...
                          className="p-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-850 dark:hover:bg-slate-850 rounded-lg text-slate-500"
                        >
                          <ZoomOut size={13} />
                        </button>
                    ...
                    {activeFile.status === "loading" && (
                      <div className="flex-1 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 gap-3 py-16">
                        ...
                        <span className="text-xs font-black uppercase tracking-wider">Parsing Document...</span>
                      </div>
                    )}

                    {/* Visual Grid Container */}
                    {activeFile.status === "ready" && activeFile.pdfjsDoc && (
                      <div className="flex-1 overflow-y-auto max-h-[500px] pr-2 custom-scrollbar border-2 border-slate-100/50 dark:border-slate-850/50 rounded-xl p-4 bg-slate-50/20 dark:bg-slate-950/10">
                          ...
                          {Array.from({ length: activeFile.pageCount || 0 }).map((_, idx) => {
                            ...
                            return (
                              <Thumbnail
                                ...
                                onToggleSelect={() => togglePageSelection(activeFile.id, idx)}
                              />
                            );
                          })}
                        </div>
                    ...
                    {/* Error display inside preview container */}
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 gap-3 py-16 text-center">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-bold">No active file selected</p>
                      <p className="text-[10px] text-slate-500">Select an uploaded PDF file from the list to view its pages.</p>
                    </div>
                  </div>
                )}
