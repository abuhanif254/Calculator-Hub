...
import { Upload, Image as ImageIcon, Download, Settings, RotateCcw, RotateCw, FlipHorizontal, FlipVertical, Crop as CropIcon, Check, Layers, RefreshCcw, Palette } from 'lucide-react';
...
function getBoundingBox(width: number, height: number, angleInDegrees: number) {
  const radians = getRadianAngle(angleInDegrees);
...
export function RotateImageTool() {
  ...
  const [canvasMode, setCanvasMode] = useState<'expand' | 'keep'>('expand');
  const [bgFill, setBgFill] = useState<'transparent' | 'white' | 'black' | 'custom'>('transparent');
  const [customBg, setCustomBg] = useState<string>('#ff0000');
    ...
    const renderPreview = async () => {
      try {
        ...
        if (canvasMode === 'expand') {
          const bbox = getBoundingBox(image.width, image.height, rotation);
          newWidth = bbox.width;
        ...
        if (bgFill !== 'transparent') {
          ctx.fillStyle = bgFill === 'custom' ? customBg : bgFill;
  ...
  }, [imageSrc, rotation, flip, canvasMode, bgFill, customBg]);
  ...
  const handleExport = async () => {
    ...
    try {
      let finalCanvas = previewCanvasRef.current;

      // If JPG and transparent background selected, JPG forces black/white anyway, but we just export the canvas.
      // If user wants White background on JPG, they should select White in the UI.
      if (exportFormat === 'image/jpeg' && bgFill === 'transparent') {
         // Create a temporary canvas with white background (JPG default behavior, but explicit is better)
      ...
      const rotatedImageUrl = finalCanvas.toDataURL(exportFormat, exportQuality);

      const link = document.createElement('a');
      link.download = `rotated-${fileName.split('.')[0]}.${exportFormat.split('/')[1]}`;
  ...
  if (!imageSrc) {
    return (
      ...
      >
        ...
        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Upload Image to Rotate</h3>
  ...
  const currentDimensions = canvasMode === 'expand' 
    ? getBoundingBox(originalSize.width, originalSize.height, rotation)
      ...
      <div className="w-full lg:w-80 flex flex-col gap-4 overflow-y-auto custom-scrollbar pr-2 h-full order-2 lg:order-1">
        ...
        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-5 border border-slate-200 dark:border-slate-700">
          <div className="flex items-center justify-between mb-3">
            ...
            <button 
              onClick={() => { setRotation(0); setFlip({ horizontal: false, vertical: false }); }}
          ...
          <div className="mb-4">
            <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400 mb-2">
              ...
              <input
                ...
                className="w-16 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded px-1 py-0.5 text-right text-slate-900 dark:text-white"
              />
              <span>°</span>
            </div>
            <input
        ...
        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-5 border border-slate-200 dark:border-slate-700">
          <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2 uppercase tracking-wider">
            <CropIcon size={16} className="text-[#518231]" /> Canvas Options
          </h3>
          
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">Canvas Size</label>
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() => setCanvasMode('expand')}
                  className={`text-xs py-2 px-2 rounded-lg border font-medium transition-all ${
                    canvasMode === 'expand'
                      ? 'bg-[#518231]/10 text-[#518231] border-[#518231]/30'
              ...
              <p className="text-[10px] text-slate-400 mt-1.5 leading-snug">
                {canvasMode === 'expand' ? "Automatically increases dimensions so corners aren't cut off." : "Maintains original image dimensions. Edges may be cropped."}
              </p>
            </div>

            <div>
              <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5 flex items-center gap-1">
                <Palette size={12} /> Fill Empty Corners
              </label>
              <div className="flex gap-2">
                <button
                  onClick={() => setBgFill('transparent')}
                  className={`flex-1 h-8 rounded-md border bg-[url('data:image/svg+xml;base64,')] ${bgFill === 'transparent' ? 'ring-2 ring-offset-1 ring-[#518231] dark:ring-offset-slate-900 border-transparent' : 'border-slate-200 dark:border-slate-700'}`}
                  title="Transparent"
                />
                <button
                  onClick={() => setBgFill('white')}
                  className={`flex-1 h-8 rounded-md border bg-white ${bgFill === 'white' ? 'ring-2 ring-offset-1 ring-[#518231] dark:ring-offset-slate-900 border-transparent' : 'border-slate-200 dark:border-slate-700'}`}
                  title="White"
                />
                <button
                  onClick={() => setBgFill('black')}
                  className={`flex-1 h-8 rounded-md border bg-black ${bgFill === 'black' ? 'ring-2 ring-offset-1 ring-[#518231] dark:ring-offset-slate-900 border-transparent' : 'border-slate-700'}`}
                  title="Black"
                />
                <div className={`relative flex-1 h-8 rounded-md border overflow-hidden ${bgFill === 'custom' ? 'ring-2 ring-offset-1 ring-[#518231] dark:ring-offset-slate-900 border-transparent' : 'border-slate-200 dark:border-slate-700'}`}>
                  <input
                    type="color"
                    value={customBg}
                    onChange={(e) => { setBgFill('custom'); setCustomBg(e.target.value); }}
      ...
      <div className="flex-1 bg-black/5 dark:bg-black/40 rounded-2xl overflow-hidden relative border border-slate-200 dark:border-slate-800 min-h-[500px] order-1 lg:order-2 flex flex-col shadow-inner">
        ...
        <div className="h-12 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 z-10">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-[#518231] font-mono bg-[#518231]/10 px-2 py-1 rounded-md border border-[#518231]/20">
              {Math.round(currentDimensions.width)} × {Math.round(currentDimensions.height)}px
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 relative bg-[url('data:image/svg+xml;base64,')] dark:bg-[url('data:image/svg+xml;base64,')] overflow-auto custom-scrollbar flex items-center justify-center p-8">
          <canvas 
            ...
            style={{ 
              // The canvas width/height attributes are native resolution.
              // CSS ensures it fits in the view while maintaining aspect ratio.
            }}
          />
        </div>
      </div>
