...
export default function HistogramChart({ data, median, mean, modes }: HistogramChartProps) {
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <ReferenceLine 
            ...
            stroke="#eab308" // Yellow for median
            strokeWidth={2}
            label={{ value: 'Median', position: 'insideTopRight', fill: '#eab308', fontSize: 12, fontWeight: 'bold' }} 
          />
          <ReferenceLine 
            x={binsData.find(b => mean >= b.binStart && mean <= b.binEnd)?.label || ''} 
            stroke="#10b981" // Green for mean
            strokeDasharray="4 4"
            strokeWidth={2}
            label={{ value: 'Mean', position: 'insideTopLeft', fill: '#10b981', fontSize: 12, fontWeight: 'bold' }} 
          />
          
          {/* We only draw a mode line if there's exactly 1 mode for clarity, otherwise it gets messy */}
          {modes.length === 1 && (
            <ReferenceLine 
              x={binsData.find(b => modes[0] >= b.binStart && modes[0] <= b.binEnd)?.label || ''} 
              stroke="#ef4444" // Red for mode
              strokeWidth={3}
              label={{ value: 'Mode', position: 'top', fill: '#ef4444', fontSize: 12, fontWeight: 'bold' }} 
            />
          )}
